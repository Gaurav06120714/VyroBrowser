...
exports.fallbackSearchUrl = fallbackSearchUrl;
...
function fallbackSearchUrl(query, engine = 'https://www.google.com/search?q=') {
    return engine + encodeURIComponent(query);
}
function buildUrl(entry, query) {
    if (query && entry.searchUrl)
        return entry.searchUrl.replace('{query}', encodeURIComponent(query));
    return entry.url;
}
...
function resolve(raw, extras = [], searchEngine = 'https://www.google.com/search?q=') {
    ...
    if (!input)
        return { type: 'none', entry: null, url: '', query: null, triggeredBy: null, score: 0, intent: null };
    if (looksLikeUrl(input)) {
        return { type: 'url', entry: null, url: normalizeUrl(input), query: null, triggeredBy: null, score: 100, intent: null };
    }
    const idx = (0, database_1.getIndex)(extras);
...
function suggest(raw, extras = [], maxResults = 8, usageCounts = new Map()) {
    ...
    const idx = (0, database_1.getIndex)(extras);
    const all = [...database_1.BUILTIN_KEYWORDS, ...extras].filter(e => e.enabled);
    const parts = lower.split(/\s+/);
    const firstWord = parts[0];
    const rest = parts.slice(1).join(' ');
    const intent = (0, intent_1.detectIntent)(lower);
    const nlp = (0, intent_1.parseNLPCommand)(lower);
    const usageBonus = (keyword) => Math.min(25, (usageCounts.get(keyword) ?? 0) * 3);
    const suggestions = [];
    ...
    if (nlp.verb !== 'none') {
        ...
        if (defaults && nlp.query) {
            for (const kw of defaults) {
                const entry = idx.get(kw);
                if (!entry || seenUrls.has(buildUrl(entry, nlp.query)))
                    continue;
                push({
                    type: 'intent',
